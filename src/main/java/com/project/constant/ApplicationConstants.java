package com.project.constant;

public class ApplicationConstants {
	public static final String EMPTY_STR = "";
	public static final String PLAN_SAVE_SUCESS = "planSaveSucess";
	public static final String PLAN_SAVE_FAIL = "planSaveFail";
	public static final String PLAN_UPDATE_SUCESS = "planUpdateSucess";
	public static final String PLAN_UPDATE_FAIL = "planUpdateFail";
	public static final String PLAN_DELETE_SUCESS = "planDeleteSucess";
	public static final String PLAN_DELETE_FAIL = "planDeleteFail";
	public static final String PLAN_STATUS_CHANGE_SUCESS = "planStatusChangeSucess";
	public static final String PLAN_STATUS_CHANGE_FAIL = "planStatusChangeFail";
	
}
